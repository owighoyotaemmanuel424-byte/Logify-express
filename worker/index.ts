...
async function handle(req: Request, env: Env) {
  ...
  if (parts.length === 2 && parts[1] === "health") return json({ ok: true, service: "logify-worker", timestamp: new Date().toISOString() });
  ...
  if (parts[1] === "auth" && parts[2] === "verify-session") { const valid = Boolean(await adminSession(req, env)); return json({ success: valid }, valid ? 200 : 401); }
